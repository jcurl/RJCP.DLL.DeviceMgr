namespace RJCP.Native
{
    ...
    internal static class Marshalling
    {
        ...
        /// the blob, separating strings by the NUL character. The end of the list has two NUL characters.
        /// </remarks>
        public static List<string> GetMultiSz(ReadOnlySpan<char> blob)
        {
            List<string> strings = new List<string>();
            int cur = 0;
            int len = blob.Length;
            ...
            while (cur < len) {
                ...
                if (nextNull < len) {
                    if (nextNull - cur > 0) {
                        strings.Add(new string(blob.Slice(cur, nextNull - cur)));
                    } else {
                        // we found an empty string.  But if we're at the end of the data,
                        // it's just the extra null terminator.
                        if (nextNull != len - 1)
                    ...
                    strings.Add(new string(blob.Slice(cur, len - cur)));
                }
                cur = nextNull + 1;
            }
            return strings;
